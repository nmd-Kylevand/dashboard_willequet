...
@section('title')
    <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
        {{$ingredientForForm->name}}: {{date("d-m-Y", strtotime(request()->date))}}
    </h2>
@endsection
@section('goBack')
...
@include('orders.partials.make-copy-individual',  ['ingredientId' => request()->id, 'clients' => $clients, 'currentDate' => request()->date])
...
<div class="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4 mb-10" role="alert">
    <p class="font-bold">Toevoegen mislukt</p>
    <p>{{$errors->first()}}</p>
</div>
@endif
    ...
    <h1 id="orderDetailTitle" class="hidden">{{date("d-m-Y", strtotime(request()->date))}}: {{$ingredientForForm->name}}</h1>
    <thead>
                   ...
                   <p style="background-color: {{$client->color ?? ''}}" id="nameColor" class="inline-block h-6 w-6 border-2 mr-4"></p><a href="/amounts/{{$client->clients_id}}"><span class="print:z-0">{{$client->name ?? ''}}</span></a>
                    ...
                    <td>
                        <div>
                            ...
                            <input type="hidden" name="ingredientOrders[]" id="ingredientOrders" value="{{$client->ingrID}}" />
                    ...
                    <td class="print:hidden">
                        ...
                        <button type="submit" name="delete" value="{{$client->ingrID}}" class=" bg-red-700 hover:bg-red-900 text-white font-bold py-2 px-4 mr-5 rounded flex"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                            </svg></button>
    ...
    <div id="etiquetDetail" class="grid grid-cols-2">
        @foreach ($orderSearchEtiq as $index => $client)
            ...
            <div class="sticker flex flex-col w-full">
                <p>
                    <span style="background-color: {{$client->color ?? ''}}" class="text-base print:z-0 p-1">{{ Str::upper($client->name) ?? '' }}</span> - {{ date('d-m', strtotime($client->date)) }}<br/>{{$client->ingredientName}}
                </p>
                <div class="flex items-center">
                    <div class="flex items-center">
                        ...
                        @endswitch
                        <p class="ml-14" style="font-size: 12px">
