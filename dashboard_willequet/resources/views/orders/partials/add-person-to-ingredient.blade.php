...
<button data-modal-target="createModal" data-modal-toggle="createModal" class="bg-blue-700 hover:bg-blue-900 text-white font-bold py-2 px-4 mx-4 rounded flex">
    ...
    <p class="ml-4">Nieuwe klant toevoegen</p>
  </button>
  <!-- Main modal -->
          ...
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <div class="p-6 space-y-6">
                  <form method="post" action="{{ route('orders.create')}}" class="mt-6 space-y-6">
                    ...
                    @method('post')
                    <div>
                        <x-text-input id="date" name="date" type="date" class="hidden" :value="request()->date"  autofocus autocomplete="date" />
                    </div>
            
                    <div>
                        <x-text-input id="ingredient" name="ingredient" type="text" class="hidden" :value="$ingredient->id"  autofocus autocomplete="ingrediënt" />
                    </div>
                    
                    <div>
                        <x-input-error class="mt-2" :messages="$errors->get('client')" />
                        <x-input-label for="client" :value="__('Klant')"/>
                        <select name="client"  style="width: 100%; " id="client" class="mt-1 client_select !w-full border-gray-300  focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm'" autofocus>
