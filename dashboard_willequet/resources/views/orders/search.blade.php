...
<tbody class="print:hidden">
    ...
    <tr>
      <td>{{$order["ingredientName"] ?? 'Add a client'}}</td>
          ...
          <td><a class="bg-blue-700 hover:bg-blue-900 text-white font-bold py-2 px-4 rounded " href="{{ url('orders/'. $order['clients'][0]->id . '-' .request()->query('search') ) }}">Detail</a></td>
...
<div id="lijst" class="orderDetailprint   !print:inline hidden">
  ...
  <h1 class="w-full" id="orderDetailTitle">{{request()->query('search')}}: {{$client["ingredientName"]}}</h1>
  ...
  <ul >
    ...
    <li >
     ...
     <div class="mt-2 mr-2">
      ...
      <div>
        <p>totaal: {{$item->totalAmount}} {{number_format($item->amount, 3, '.', ',')}}pp</span></p>
      </div>
      @if ($item->comment)
      <div>
        <p> - </p>
...
<ul id="etiquet" class="orderDetailEtiq hidden">
  ...
  @foreach ($orderSearchEtiq as $client)
      <div class="flex flex-col w-full p-5 gap-1">
        <p class="text-lg">
