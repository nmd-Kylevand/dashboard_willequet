...
@section('printButton')
<x-print-button/>
<button class="bg-blue-700 hover:bg-blue-900 text-white font-bold py-2 px-4 mx-4  rounded flex" onclick="printEtiquet()" id="button">
...
<ul id="lijst" class="orderDetailprint !print:inline hidden">
  ...
  <h1 id="orderDetailTitle">{{$client->clientsOrders[0]->pivot->date}}: {{$client->name}}</h1>
  @foreach ($client->clientsOrders as $item)
  <li><span style="background-color: {{$item->color ?? ''}}" class="print:z-0 ">{{Str::upper($item->name) ?? ''}}</span><span class="numbersSpan">{{$item->pivot->cups}} totaal: {{$item->pivot->totalAmount}} {{$item->pivot->amount}}pp</span></li>
      
  @endforeach
  
  @endforeach
</ul>
<ul id="etiquet" class="orderDetailEtiq print:inline!important hidden">
  @foreach ($orderSearch as $client)
  <h1 id="orderDetailTitle">{{$client->clientsOrders[0]->pivot->date}}: {{$client->name}}</h1>
  <div class="grid grid-cols-2 pt-8 w-full">
  @foreach ($client->clientsOrders as $item)
    <div class="flex flex-col w-full p-6 gap-8 ">
      <p class="text-lg">
        <span style="background-color: {{$item->color ?? ''}}" class="print:z-0 ">{{Str::upper($item->name) ?? ''}}-{{$client->clientsOrders[0]->pivot->date}}</span>
    </p>
